...
var Cart = function(){
	...
	this.update = function(options){
		if(typeof options == "undefined"){
			...
			$(".cart .total").html(numberFormat(options.total,0,'.',' '));
...
var Modal = function(name,url){
	...
	this.listUpdate = function(options){
		...
		if(options.item.count != 0){
			$tr.find('.total').html(options.item.total);
			$tr.find('.count').html(options.item.count);
		}else{
			$tr.remove();
		}
		this.$modal.find('.total-price').html(numberFormat(options.total,0,'.',' '));
	}
}
